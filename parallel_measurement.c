/*
   Parallel implementation of plotting fractals using Mandelbrot Set
 */
...
PPMImage* plot();
  ...
  int c;
  int c_max;
  int count_max = 5000;		// max number iterations for which we calculate the Mandelbrot function values for each pixel
  int i;
  int j;
  int k;
  double wtime;
...
# pragma omp parallel \
  shared ( count, count_max, x_max, x_min, y_max, y_min ) \
